...
def add_listening_sentence(sentence,tokens,result):
    mydb = get_connection()
    mycursor = mydb.cursor()
    sql = "insert into listen_sentence(listened_at,sentence,tokens,result) values (UTC_TIMESTAMP(),'"+escape_sql_string(sentence)+"','"+escape_sql_string(tokens)+"',"+str(result)+")"
    mycursor.execute(sql)
    mydb.commit()
    mycursor.close()
    mydb.close()
...
def get_failed_outputs_lately(nr,days):
    ...
    for (chinesetokens,english,timewhen) in myresult:
        result.append({"english":english,"chinese": json.loads(chinesetokens) })
    new_array = random.sample(result,2000-20)
    result = new_array
    mycursor.close()
    mydb.close()
    return result
